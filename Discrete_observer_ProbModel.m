%Discrete case
%Observer makes actions according to:
% a ~ P(r=1|a,s)P(adem|s)P(a) / Z
...
Pescape = [0.5; 0.5]; %prior probability of escape
...
R = nan(1,ntrials); %rewards earned
P = nan(2,ntrials); %posterior probability over actions given stimulus in x
K = S;
...
for ii = 1:ntrials
    ...
    %choose stochastic action according to scheme above
    p = exp(beta*Q(:,h)).*pid_as(:,h).*Pescape;
    P(:,h) = p/sum(p);
    a(ii) = 1 + double(P(2,ii)> (rand));
    %for greedy action:
    %[~,a(ii)] = max(pio(:,h));
    
%      %choose a sigma greedy action
